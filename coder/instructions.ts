...
function encodeInitialize2({nonce, openTime, initPcAmount, initCoinAmount,}: any): Buffer {
    return encodeData({initialize2: {nonce, openTime, initPcAmount, initCoinAmount,}}, 1 + 1 + 8 + 8 + 8)
}

function encodeMonitorStep({planOrderLimit, placeOrderLimit, cancelOrderLimit,}: any): Buffer {
...
function encodeSetParams({param, value, newPubkey, fees, lastOrderDistance, needTakeAmounts,}: any): Buffer {
    return encodeData({
        setParams: {
            param,
            value,
    ...
    }, 1 + 1 + 1 + (value === null ? 0 : 8) + 1 + (newPubkey === null ? 0 : 32) + 1 + (fees === null ? 0 : 64) + 1 + (lastOrderDistance === null ? 0 : 16) + 1 + (needTakeAmounts === null ? 0 : 16))
}
...
function encodeSimulateInfo({param, swapBaseInValue, swapBaseOutValue,}: any): Buffer {
    return encodeData({
        simulateInfo: {
            param,
            swapBaseInValue,
    ...
    }, 1 + 1 + 1 + (swapBaseInValue === null ? 0 : 16) + 1 + (swapBaseOutValue === null ? 0 : 16))
}
...
LAYOUT.addVariant(12, B.struct([B.u8('param'), B.option(B.struct([B.u64('amountIn'), B.u64('minimumAmountOut'),],), 'swapBaseInValue'), B.option(B.struct([B.u64('maxAmountIn'), B.u64('amountOut'),],), 'swapBaseOutValue'),]), 'simulateInfo')
